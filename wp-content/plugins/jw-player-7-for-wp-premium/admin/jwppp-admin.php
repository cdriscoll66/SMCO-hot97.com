...
function jwppp_options() {
	...
	echo '<div id="jwppp-description">';
		echo '<h1 class="jwppp main">' . esc_html( __( 'JW Player for WordPress - Premium', 'jwppp' ) ) . '</h1>';
	echo '</div>';
	...
	<h2 id="jwppp-admin-menu" class="nav-tab-wrapper">
	 ...
	 <div name="jwppp-settings" id="jwppp-settings" class="jwppp-admin" style="display: block;">
		...
		if( isset( $_POST['jwppp-premium-key'] ) && wp_verify_nonce( $_POST['hidden-nonce-options'], 'jwppp-nonce-options') ) {
			$key = sanitize_text_field( wp_unslash( $_POST['jwppp-premium-key'] ) );
		...
		echo '<p class="description">' . esc_html( __( 'The Premium key received by purchasing this plugin', 'jwppp' ) ) . '</p>';
		echo '</td>';
		echo '</tr>';
